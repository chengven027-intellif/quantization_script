...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        evaluate(model, criterion, data_loader_test, neval_batches=20)

    # Save the model
    quant_nn.TensorQuantizer.use_fb_fake_quant = True
    example_inputs = torch.randn(1, 3, 224, 224)
    trt_model_file_path_torch = saved_model_dir + "resnet50_trt_quantized.pth"
    trt_model_file_path_onnx = saved_model_dir + "resnet50_trt_quantized.onnx"
    ...
    # example_inputs = torch.randn(1, 3, 224, 224)
    # onnx_model = onnx.load("./data/resnet50_trt_quantized.onnx")
    # mod, params = relay.frontend.from_onnx(
    #     onnx_model, {"inputs.1": example_inputs.numpy().shape}, {"x": "float32"}
    # )
    # mod = relay.transform.InferType()(mod)
    # print(mod)
    # print("*************Print Onnx IR Success************************")