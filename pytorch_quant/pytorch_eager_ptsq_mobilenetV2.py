...
output_names = ["output"]
scripted_model = torch.jit.trace(per_channel_quantized_model.eval(), input_fp32)
torch.jit.save(scripted_model, "./data/eager_ptsq.torchscript")
torch.onnx.export(per_channel_quantized_model, 
                  input_fp32,
                  "./data/eager_ptsq.onnx",
                  verbose=False,
...
torchscript_model = torch.jit.load("./data/eager_ptsq.torchscript")
mod, params = relay.frontend.from_pytorch(torchscript_model, shape_list)
print(mod)
print("*************Print Pytorch IR Success************************")

onnx_model = onnx.load("./data/eager_ptsq.onnx")
